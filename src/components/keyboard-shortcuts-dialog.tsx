...
function Kbd({ children, small }: { children: React.ReactNode; small?: boolean }) {
  return (
    <kbd
      className={cn(
        'bg-muted/80 border border-border/80 rounded px-1.5 py-0.5 text-sm font-mono inline-flex items-center gap-0.5 shadow-sm select-none',
        small && 'px-1 text-xs'
      )}
    >
      {children}
          ...
          <div className="p-6 space-y-5 max-h-[400px] overflow-y-auto custom-scrollbar">
            {shortcutGroups.map((group, groupIndex) => (
              <div key={group.title}>
                ...
                <h3 className="text-sm uppercase tracking-wider font-semibold text-muted-foreground mb-2 px-2">
                  {group.title}
                </h3>
